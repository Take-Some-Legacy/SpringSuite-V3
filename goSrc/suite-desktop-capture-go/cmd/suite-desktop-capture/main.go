...
func screenshotCommand(args []string, out io.Writer) error {
    ...
    if *maxWidth > 0 && originalWidth > *maxWidth { final = resizeNearest(img, *maxWidth) }

    var buf bytes.Buffer
    if err := png.Encode(&buf, final); err != nil { return err }
    data := buf.Bytes()
    if strings.TrimSpace(*output) != "" {
        if err := os.WriteFile(*output, data, 0600); err != nil { return err }
    }
    if !*jsonOut { _, err := out.Write(data); return err }
    ...
    if *includeBase64 { result.Base64 = base64.StdEncoding.EncodeToString(data) }
    enc := json.NewEncoder(out); enc.SetEscapeHTML(false); return enc.Encode(result)
}

func captureImage(target string) (image.Image, string, error) {
    target = strings.ToLower(strings.TrimSpace(target))
    if target == "" || target == "desktop" || target == "all" { target = "virtual" }
    count := screenshot.NumActiveDisplays()
    if count <= 0 { return nil, target, fmt.Errorf("no active displays detected") }
    if target == "primary" {
        bounds := screenshot.GetDisplayBounds(0)
        img, err := screenshot.CaptureRect(bounds)
        return img, "primary", err
    }
    if target != "virtual" { return nil, target, fmt.Errorf("unsupported target: %s", target) }
    union := screenshot.GetDisplayBounds(0)
    for i := 1; i < count; i++ { union = union.Union(screenshot.GetDisplayBounds(i)) }
    dst := image.NewRGBA(image.Rect(0, 0, union.Dx(), union.Dy()))
    for i := 0; i < count; i++ {
        bounds := screenshot.GetDisplayBounds(i)
        img, err := screenshot.CaptureRect(bounds)
        if err != nil { return nil, "virtual", err }
...
func resizeNearest(src image.Image, maxWidth int) image.Image {
    b := src.Bounds(); srcW := b.Dx(); srcH := b.Dy()
    if maxWidth <= 0 || srcW <= maxWidth { return src }
    dstW := maxWidth
...
func doctor(out io.Writer) error {
    ...
    enc := json.NewEncoder(out); enc.SetEscapeHTML(false); return enc.Encode(info)
}
