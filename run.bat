...
pushd "%ROOT%"
start "SpringSuite" "%JAVA_EXE%" "-XX:ErrorFile=%ROOT%\logs\crash\hs_err_pid%%p.log" -XX:+ShowCodeDetailsInExceptionMessages -Dfile.encoding=UTF-8 -Dstdout.encoding=UTF-8 -Dstderr.encoding=UTF-8 -Dsun.stdout.encoding=UTF-8 -Dsun.stderr.encoding=UTF-8 -jar "%JAR%" "--suite-working-directory=%ROOT%" %*
popd
endlocal
