...
tasks.named<Jar>("jar") {
    archiveBaseName.set("spring-suite-diagnostics-module")
    manifest {
        attributes(
            "SpringSuite-Module" to "spring-suite-diagnostics",
            "SpringSuite-Module-Version" to project.version.toString(),
            "Implementation-Title" to "SpringSuite Diagnostics Module",
...
val moduleKeystore = layout.buildDirectory.file("module-signing/spring-suite-module-dev.jks")
...
val generateModuleSigningKeystore = tasks.register<Exec>("generateModuleSigningKeystore") {
    group = "modules"
    description = "Generate a local development keystore for signing runtime modules."
    outputs.file(moduleKeystore)
    onlyIf { !moduleKeystore.get().asFile.isFile }
    doFirst {
        moduleKeystore.get().asFile.parentFile.mkdirs()
    }
    commandLine(
        "keytool",
        "-genkeypair",
...
val signModuleJar = tasks.register<Exec>("signModuleJar") {
    group = "modules"
    description = "Build and sign the diagnostics runtime module as an external module jar."
    dependsOn(tasks.named("jar"), generateModuleSigningKeystore)
...
tasks.register<Copy>("deploySignedModule") {
    group = "modules"
    description = "Copy the signed diagnostics module jar into the external runtime modules directory."
