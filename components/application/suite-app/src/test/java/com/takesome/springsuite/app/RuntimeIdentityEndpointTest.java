...
class RuntimeIdentityEndpointTest {
    ...
    void exposesIdentityExpectedByRuntimeController() {
        ...
        try {
            ...
            assertThat(components.get("uptimeMillis")).isInstanceOf(Long.class);
            assertThat((Long) components.get("uptimeMillis")).isGreaterThanOrEqualTo(0L);
            assertThat(components.get("heapUsedBytes")).isInstanceOf(Long.class);
            assertThat((Long) components.get("heapUsedBytes")).isGreaterThan(0L);
            assertThat(components.get("threadCount")).isInstanceOf(Integer.class);
            assertThat((Integer) components.get("threadCount")).isGreaterThan(0);
            assertThat(components.get("availableProcessors")).isInstanceOf(Integer.class);
            assertThat((Integer) components.get("availableProcessors")).isGreaterThan(0);
            assertThat(components).containsKeys("processCpuLoad", "systemCpuLoad", "systemLoadAverage");
        } finally {
