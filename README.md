...
The browser control panel exposes the same data in the **SQL Request Journal** section with full-text search, filters, paging and record inspection. The **Enable notifications** action requests the browser permission once, stores the local preference and subscribes to the SSE stream. Browser Notifications are processed through a bounded queue: the first event is shown immediately, later events rotate at most once per second, and only one notification remains visible at a time. The notification lifetime is selectable in the control panel (3, 5, 8, 15 or 30 seconds; default 8 seconds) and is stored in browser `localStorage`. Clicking a notification focuses the panel and opens that request record. The panel page must remain open, while it may be minimized or in a background tab. Journal administration endpoints and the operator SSE stream are excluded from capture to prevent recursive/self-generated traffic.
