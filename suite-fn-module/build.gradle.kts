...
dependencies {
    ...
    compileOnly(project(":suite-config"))
}

tasks.named<Jar>("jar") {
    archiveBaseName.set("spring-suite-fn-module")
    manifest {
        attributes(
            "SpringSuite-Module" to "spring-suite-fn",
            "SpringSuite-Module-Version" to project.version.toString(),
            "Implementation-Title" to "SpringSuite FN Operator Module",
...
val externalV3ModulesDir = providers.gradleProperty("suite.v3.modules.dir")
    .orElse("C:\\Users\\Aiden\\Documents\\Take Some\\NorthStar-Suite-V3\\modules")
...
val signModuleJar = tasks.register<Exec>("signModuleJar") {
    group = "modules"
    description = "Build and sign the FN operator runtime module as an external module jar."
...
tasks.register<Copy>("deploySignedModule") {
    group = "modules"
    description = "Copy the signed FN module jar into the external NorthStar-Suite-V3 modules directory."
    dependsOn(signModuleJar)
    from(signedModuleJar)
    into(file(externalV3ModulesDir.get()))
}
